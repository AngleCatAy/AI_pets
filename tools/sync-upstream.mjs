...
const PATCHES = [
  ...
  {
    // 峰谷倒计时按模型算：这段推算原先写死 DeepSeek 的时段表（工作日 9–12 + 14–18），
    // 而 GLM 的高峰只有工作日 14–18 —— 直接套用会把「距下次切换」算错。
    // 谷/峰状态字本身不用这里（优先用服务端传的 state.isPeak），但倒计时模块
    // （peakStyle:'count'）完全依赖这两个函数。
    // 切换点扫描的边界表 [0,9,12,14,18] 两边都能用：对 GLM 而言 9/12 点两侧同为谷，
    // 判等后自动跳过，真正的切换点仍是 14:00 / 18:00。
    from: '  if (dow === 0 || dow === 6) return false\n' +
      '  return (h >= 9 && h < 12) || (h >= 14 && h < 18)\n' +
      '}',
    to: '  if (dow === 0 || dow === 6) return false\n' +
      '  // GLM（智谱）：高峰只有工作日下午 14:00–18:00，其余含周末全天为谷\n' +
      "  if (state && state.provider === 'glm') return h >= 14 && h < 18\n" +
      '  return (h >= 9 && h < 12) || (h >= 14 && h < 18)\n' +
      '}',
    why: '峰谷倒计时按模型算（GLM 高峰只有工作日 14–18，DeepSeek 是 9–12 与 14–18）',
  },
  {
