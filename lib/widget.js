...
// 独立版：宿主是普通窗口，没有 DSH 的 #root composer，所以不做这项自检
...
var css = [
  ...
  '.dshwv-menu-btn{position:absolute;top:calc(40.55% + 4px);right:4px;width:26px;height:26px;border:2px solid rgba(255,255,255,.92);border-radius:6px;background:rgba(32,49,112,.85);box-shadow:0 1px 4px rgba(0,0,0,.5);cursor:pointer;pointer-events:auto;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;padding:0;z-index:2;opacity:0;transition:opacity .15s ease}',
...
function bubbleModuleSummary(m) {
  ...
  if (m.type === 'random') return (m.name || '随机语句') + (m.lines && m.lines.length ? '(' + m.lines.length + '条)' : '(空)')
...
var RANDOM_GROUPS = [
  { w: 7, ds: true, lines: function () { return singleCenter('B', pickOne(['好模型... ↓', '压力一只蓝色大肥鱼？！'])) } },
  { w: 7, ds: true, lines: function () { return singleCenter('A', pickOne(['不知道用户有什么用，先赶走吧~', '我...我...我也要挣钱吗？', '我去吃饭啦，测完叫我', '好女孩...↓', 'DeepSleep...', '坏了...用户彻底怒了！']), '', true) } },
  { w: 10, ds: true, lines: function () { return { gif: true } } },
  { w: 3, ds: true, lines: function () { return singleCenter('A', pickOne(['你目录里的dsh是什么...大烧货吗...?', '恭喜你实现token自由！token全跑了！', '真当我是便宜货啊...']), '', true) } },
  { w: 1, ds: true, lines: function () { return singleCenter('B', '哦鲸鲸... ') } },
]
...
function fmt(balance, currency) {
  ...
  if (currency === '%') return Math.round(num) + '%'
...
function render() {
  ...
  if (state.status === 'error') {
    ...
    hint = (state.usageLabel || '今日已用') + ' ' + (state.todayUsage !== null && state.todayUsage !== undefined ? fmt(state.todayUsage, state.currency) : '--')
  }
...
function renderRolePanel() {
  try {
    ...
    roleList.forEach(function (r) {
      ...
      // 默认角色贴图 = image.png（按当前模型出图）：渲染时带 bust，否则
...
function openCropModal(dataUrl, fileName) {
  try {
    ...
    imgEl.onload = function () {
      ...
      // 角色名默认用导入的文件名（去扩展名；超长由输入框 maxLength 截断），
...
function onDocClickStopper(e) {
  ...
  // 菜单按钮和菜单面板上的点击必须放行：它们压在贴图不透明像素上时
...
window.__dshWhaleApi = {
  showRandomLine: function () {
    try {
      ...
      var lines = state.provider === 'glm'
        ? [null, { t: '当前时间段为:', s: 'A' },
           { t: state.isPeak ? '高峰时段' : '空闲时段', s: 'P', c: state.isPeak ? '#e0433f' : '#2fa24c' }]
        : pickRandomLines()
      bubbleRandomLines = lines
      // 必须走 widget 自己的场景机制：sceneOpen 会先 bubbleClearModuleRows()
      // 清掉上一次（自定义泡泡序列）留下的 .dshwv-trow 模块行。
      // 曾经这里是 showBubble() + applyBubbleLines()：showBubble 会先把序列第 1 泡
      // 的模块行渲染出来，而我们再往老三行写台词、两边同时可见 → 文本叠在一起、
      // 溢出泡泡框（用户实测，DeepSeek 与 GLM 都会）。
      sceneOpen('random', function () { bubbleRenderRandom(lines) }, BUBBLE_MS)
      return true
    } catch (err) {
      return false
